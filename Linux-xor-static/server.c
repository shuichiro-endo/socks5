/*
 * Title:  socks5 server (Linux static)
...
int forwarderXorWorker2(void *ptr)
{
	...
	while(1){
		...
		rec = recvData(targetSock, buffer, BUFSIZ, tv_sec, tv_usec);
		if(rec > 0){
			sen = sendDataXor(clientSock, buffer, rec, tv_sec, tv_usec);
			if(sen <= 0){
				break;
			}
		}else{
...
int worker(void *ptr)
{
	...
	struct sockaddr_in targetAddr;		// IPv4
	memset(&targetAddr, 0, sizeof(struct sockaddr_in));
	
	struct sockaddr_in6 targetAddr6;	// IPv6
	...
	if(pSocksRequest->atyp == 0x1){	// IPv4
		...
		if(colon == NULL){	// ipv4 address or domainname
			...
			snprintf(portNumber, sizeof(portNumber), "%d", ntohs(targetAddr.sin_port));
			ret = getAddrInfo(domainname, portNumber, AF_INET, &targetAddr, NULL);
			if(ret != 0){
				family = AF_INET6;	// IPv6
				memcpy(&targetAddr6.sin6_port, &pSocksRequestDomainname->dstAddr[domainnameLength], 2);
				snprintf(portNumber, sizeof(portNumber), "%d", ntohs(targetAddr6.sin6_port));
				ret = getAddrInfo(domainname, portNumber, AF_INET6, NULL, &targetAddr6);
				if(ret != 0){
#ifdef _DEBUG
...
int main(int argc, char **argv)
{
	...
	if(clientPortNumber != NULL){
		clientPort = (uint16_t)atoi(clientPortNumber);
	}
	...
	if(reverseFlag == 0){	// Nomal mode
		...
		if(colon == NULL){	// ipv4 address or domainname
			family = AF_INET;	// IPv4
			ret = getAddrInfo(clientDomainname, clientPortNumber, AF_INET, &clientAddr, NULL);
			if(ret != 0){
				family = AF_INET6;	// IPv6
				ret = getAddrInfo(clientDomainname, clientPortNumber, AF_INET6, NULL, &clientAddr6);
				if(ret != 0){
#ifdef _DEBUG
					printf("[E] Cannot resolv the domain name:%s\n", clientDomainname);
#endif
			...
			family = AF_INET6;	// IPv6
			scopeId = getIpv6ScopeId(clientDomainname);
			if(scopeId == 0){
				ret = getAddrInfo(clientDomainname, clientPortNumber, AF_INET6, NULL, &clientAddr6);
				if(ret != 0){
#ifdef _DEBUG
				...
				addr = getIpv6AddrString(clientDomainname);
				if(addr == NULL){
#ifdef _DEBUG
					printf("[E] Cannot resolv the domain name:%s\n", clientDomainname);
#endif
				...
				ret = getAddrInfo(addr, clientPortNumber, AF_INET6, NULL, &clientAddr6);
				if(ret != 0){
#ifdef _DEBUG
					printf("[E] Cannot resolv the domain name:%s\n", clientDomainname);
#endif
				...
				clientAddr6.sin6_scope_id = scopeId;
				free(addr);
			}
		}
